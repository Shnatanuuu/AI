...
TRANSLATIONS = {
    "English": {
        ...
        "ai_photos_of_faults": "AI - Photos of Faults",
...
# Photos of Faults storage - NEW
if 'ai_defect_containers' not in st.session_state:
    st.session_state.ai_defect_containers = {}  # {container_id: {'name': '', 'severity': '', 'images': []}}
if 'qc_defect_containers' not in st.session_state:
    st.session_state.qc_defect_containers = {}  # {container_id: {'name': '', 'severity': '', 'images': []}}
...
def generate_qc_report(analyses):
    ...
    # Create AI defect containers from analyzed defects
    create_ai_defect_containers(analyses)
...
def create_ai_defect_containers(analyses):
    """Create AI defect containers from analysis results"""
    st.session_state.ai_defect_containers = {}
    
    # Process all analyses to create defect containers
    container_counter = 1
    for idx, analysis in enumerate(analyses):
        if not analysis:
            continue
            
        # Process critical defects
        for defect in analysis.get('critical_defects', []):
            container_id = f"ai_critical_{container_counter}"
            st.session_state.ai_defect_containers[container_id] = {
                'name': defect,
                'severity': 'critical',
                'images': []
            }
        ...
        # Process major defects
        for defect in analysis.get('major_defects', []):
            container_id = f"ai_major_{container_counter}"
            st.session_state.ai_defect_containers[container_id] = {
                'name': defect,
                'severity': 'major',
                'images': []
        ...
        # Process minor defects
        for defect in analysis.get('minor_defects', []):
            container_id = f"ai_minor_{container_counter}"
            st.session_state.ai_defect_containers[container_id] = {
                'name': defect,
                'severity': 'minor',
                'images': []
...
def create_sampling_table(order_qty, language, chinese_font=None):
    ...
    try:
        ...
        if language == "Mandarin":
            # FIX: Proper Chinese characters for "Inspection Results Total QTY"
            data = [
                # Title row
                ['抽样检查标准 2.5', '', '', '', ''],
                # Header row
                ['批次数量', '检查数量', '严重问题AQL', '主要问题AQL', '次要问题AQL'],
                # Data rows
                ['0-300双', '100%', '0', '1', '5'],
                ['301-1200双', '80', '0', '5', '9'],
                ['1201-3200双', '125', '0', '7', '10'],
                ['3201-10000双', '200', '0', '10', '14'],
                ['10001-35000双以上', '315', '0', '14', '21'],
                # Last row with actual results - FIXED MANDARIN TEXT
                [f"检查结果总数量: {order_qty}", 
            ...
            data = [
                ...
                [f"Inspection Results Total QTY: {order_qty}", 
...
def create_problem_table(problem_data, title, language, chinese_font=None):
    ...
    style = TableStyle([
        ...
        # Header row - bold
        ('FONTNAME', (0, 0), (7, 0), 'Helvetica-Bold' if language == "English" else chinese_font if chinese_font else 'Helvetica'),
        ('FONTSIZE', (0, 0), (7, 0), 8),
        
        # Last row - bold
        ('FONTNAME', (0, 13), (3, 13), 'Helvetica-Bold' if language == "English" else chinese_font if chinese_font else 'Helvetica'),
        ('FONTNAME', (4, 13), (4, 13), 'Helvetica-Bold' if language == "English" else chinese_font if chinese_font else 'Helvetica'),
        ('FONTNAME', (5, 13), (7, 13), 'Helvetica-Bold' if language == "English" else chinese_font if chinese_font else 'Helvetica'),
...
def create_photos_of_faults_section(container_data, title, language):
    """Create Photos of Faults section for PDF"""
    elements = []
    ...
    critical_containers = {k: v for k, v in container_data.items() if v['severity'] == 'critical'}
    major_containers = {k: v for k, v in container_data.items() if v['severity'] == 'major'}
    minor_containers = {k: v for k, v in container_data.items() if v['severity'] == 'minor'}
    
    # Title
    title_style = ParagraphStyle('PhotosTitle', parent=getSampleStyleSheet()['Heading2'], 
                               fontSize=12, alignment=TA_CENTER, spaceAfter=12)
    
    if language == "Mandarin":
        title_text = f"{title} - 缺陷照片"
    else:
        title_text = f"{title} - Photos of Faults"
    ...
    if critical_containers:
        if language == "Mandarin":
            section_title = "严重缺陷照片"
        else:
            section_title = "Critical Defects Photos"
        
        section_style = ParagraphStyle('CriticalSection', parent=getSampleStyleSheet()['Heading3'], 
                                     fontSize=10, textColor=colors.red, spaceAfter=8)
        elements.append(Paragraph(section_title, section_style))
        
        for container_id, container in critical_containers.items():
            if container['images']:
                # Defect name
                name_style = ParagraphStyle('DefectName', parent=getSampleStyleSheet()['Normal'], 
                                          fontSize=9, textColor=colors.red, spaceAfter=4)
                elements.append(Paragraph(f"• {container['name']}", name_style))
                
                # Create image grid
                cols = 3
                images_per_row = []
                
                for img_idx, img_data in enumerate(container['images']):
                    try:
                        img_buffer = io.BytesIO(img_data)
                        img_buffer.seek(0)
                        reportlab_img = ReportLabImage(img_buffer, width=3*cm, height=3*cm)
                        images_per_row.append(reportlab_img)
                        
                        # Add to table every 'cols' images or at the end
                        if len(images_per_row) == cols or img_idx == len(container['images']) - 1:
                            # Pad the row if necessary
                            while len(images_per_row) < cols:
                                images_per_row.append(Spacer(3*cm, 3*cm))
                            
                            img_table = Table([images_per_row], colWidths=[3*cm]*cols)
                        ...
                        st.warning(f"Could not add image to PDF: {str(e)}")
    
    # Major defects section
    if major_containers:
        if language == "Mandarin":
            section_title = "主要缺陷照片"
        else:
            section_title = "Major Defects Photos"
        
        section_style = ParagraphStyle('MajorSection', parent=getSampleStyleSheet()['Heading3'], 
                                     fontSize=10, textColor=colors.orange, spaceAfter=8)
        elements.append(Paragraph(section_title, section_style))
        
        for container_id, container in major_containers.items():
            if container['images']:
                # Defect name
                name_style = ParagraphStyle('DefectName', parent=getSampleStyleSheet()['Normal'], 
                                          fontSize=9, textColor=colors.orange, spaceAfter=4)
                elements.append(Paragraph(f"• {container['name']}", name_style))
    ...
    if minor_containers:
        if language == "Mandarin":
            section_title = "次要缺陷照片"
        else:
            section_title = "Minor Defects Photos"
        
        section_style = ParagraphStyle('MinorSection', parent=getSampleStyleSheet()['Heading3'], 
                                     fontSize=10, textColor=colors.blue, spaceAfter=8)
        elements.append(Paragraph(section_title, section_style))
        
        for container_id, container in minor_containers.items():
            if container['images']:
                # Defect name
                name_style = ParagraphStyle('DefectName', parent=getSampleStyleSheet()['Normal'], 
                                          fontSize=9, textColor=colors.blue, spaceAfter=4)
                elements.append(Paragraph(f"• {container['name']}", name_style))
    ...
    if not (critical_containers or major_containers or minor_containers):
        if language == "Mandarin":
            no_photos_text = "暂无缺陷照片"
        else:
            no_photos_text = "No photos of faults available"
        
        no_photos_style = ParagraphStyle('NoPhotos', parent=getSampleStyleSheet()['Normal'], 
                                        fontSize=9, alignment=TA_CENTER, textColor=colors.grey)
        elements.append(Paragraph(no_photos_text, no_photos_style))
    
    elements.append(Spacer(1, 20))
    return elements

def generate_multilingual_pdf(order_info, language):
    """Generate PDF with proper Mandarin translation and font handling"""
    try:
        buffer = io.BytesIO()
        ...
        # Register Chinese font for Mandarin PDF
        if language == "Mandarin":
            try:
                ...
                try:
                    ...
                    if sys.platform == "win32":
                        ...
                        chinese_font_path = "/System/Library/Fonts/PingFang.ttc"  # PingFang
                    else:
                        chinese_font_path = "/usr/share/fonts/truetype/wqy/wqy-microhei.ttc"  # WenQuanYi
                    ...
                    if os.path.exists(chinese_font_path):
                        ...
                        # Fallback to STSong if available
                ...
                st.warning(f"Chinese font not available: {str(e)}. Using default font.")
        ...
        # Styles - adjusted for Chinese fonts
        ...
        # Header - TRANSLATED FOR MANDARIN
        if language == "Mandarin":
        ...
        # SAMPLING INSPECTION TABLE - Add page break if needed
        ...
        # PROBLEM IDENTIFIED BY THE QC MANAGER TABLE
        if language == "Mandarin":
        ...
        # QC INSPECTOR REVIEW & AMENDMENTS
        if language == "Mandarin":
            qc_label = "质检员检查与修改"
        else:
        ...
        if language == "Mandarin":
            ...
            qc_data = [
                ...
                ['Major', str(ai_major_count), str(qc_major_count),
                 f"{maj_change:+d}" if maj_change != 0 else "0",
                 'FAIL' if qc_major_count>10 else 'PASS'],
                ['Minor', str(ai_minor_count), str(qc_minor_count),
                 f"{min_change:+d}" if min_change != 0 else "0",
                 'FAIL' if qc_minor_count>14 else 'PASS']
            ]
        ...
        # PHOTOS OF FAULTS - AI
        elements.append(PageBreak())
        photos_elements_ai = create_photos_of_faults_section(st.session_state.ai_defect_containers, "AI", language)
        elements.extend(photos_elements_ai)
        
        # PHOTOS OF FAULTS - QC MANAGER
        elements.append(PageBreak())
        photos_elements_qc = create_photos_of_faults_section(st.session_state.qc_defect_containers, "QC Manager", language)
        elements.extend(photos_elements_qc)
        ...
        if language == "Mandarin":
            signature_label = "签名"
            signature_data = [
                ['制造商签名', "_________________________"],
                ['检查员签名', "_________________________"]
            ]
        else:
            signature_label = "SIGNATURES"
            signature_data = [
                ['Manufacturer Signature', "_________________________"],
...
def render_defect_container_ui(container_type):
    """Render UI for defect containers (Photos of Faults)"""
    if container_type == "ai":
        containers = st.session_state.ai_defect_containers
        title = t("ai_photos_of_faults")
        key_prefix = "ai_"
    else:
        containers = st.session_state.qc_defect_containers
        title = t("qc_photos_of_faults")
        key_prefix = "qc_"
    ...
    with st.expander(t("create_new_defect_container"), expanded=False):
        ...
        with col1:
            new_container_name = st.text_input(t("container_name"), key=f"new_{key_prefix}container_name")
        with col2:
            new_container_severity = st.selectbox(
                t("defect_severity"),
                ['critical', 'major', 'minor'],
                format_func=lambda x: t(x.upper()),
                key=f"new_{key_prefix}container_severity"
            )
        with col3:
            st.write("")  # Spacer
            st.write("")  # Spacer
            if st.button(t("add_defect_container"), key=f"add_{key_prefix}container"):
                if new_container_name.strip():
                    container_id = f"{key_prefix}container_{len(containers)}_{datetime.now().timestamp()}"
                    containers[container_id] = {
                        'name': new_container_name.strip(),
                        'severity': new_container_severity,
                        'images': []
                    }
                    st.success(f"Container created for: {new_container_name.strip()}")
                    st.rerun()
    ...
    if containers:
        for container_id, container in containers.items():
            ...
            with col2:
                if st.button(f"❌ Remove", key=f"remove_{container_id}"):
                    del containers[container_id]
                    st.rerun()
            
            # Upload images for this container
            uploaded_files = st.file_uploader(
                t("upload_defect_image"),
                type=['png', 'jpg', 'jpeg'],
                accept_multiple_files=True,
                key=f"upload_{container_id}"
            )
            
            if uploaded_files:
                for uploaded_file in uploaded_files:
                    if uploaded_file not in [img['file'] for img in container['images'] if 'file' in img]:
            ...
            if container['images']:
                cols = st.columns(3)
                for idx, img_data in enumerate(container['images']):
                    with cols[idx % 3]:
                        st.image(Image.open(io.BytesIO(img_data['bytes'])), use_container_width=True)
                        if st.button(f"❌ {t('remove_image')}", key=f"remove_img_{container_id}_{idx}"):
        ...
        st.info(f"No defect containers created yet for {title}")
...
def render_problem_table_ui(which_table="ai"):
    ...
    if st.session_state.ui_language == "Mandarin":
        problem_names = {
            ...
            'adhesion': '胶着力',
            'buckle': '鞋扣',
            'midsole_glue': '中底布欠胶开胶皱',
            'tongue': '鞋舌',
            'grinding_high': '帮脚打磨高',
            'back_strap_length': '后带长',
            'heel': '鞋跟',
            'back_strap_attachment': '后带固定',
            'toplift': '大皮',
            'damage_upper': '鞋面受损',
            'bottom_gapping': '底开胶',
            'xray_wrinkle': '鞋面打皱',
    ...
    for left_key, right_key in problem_pairs:
        ...
        editing_left_cr = (st.session_state.editing_problem_table == (which_table, left_key, 'cr') if left_key else False)
        editing_left_major = (st.session_state.editing_problem_table == (which_table, left_key, 'major') if left_key else False)
        editing_left_minor = (st.session_state.editing_problem_table == (which_table, left_key, 'minor') if left_key else False)
        
        editing_right_cr = (st.session_state.editing_problem_table == (which_table, right_key, 'cr') if right_key else False)
        editing_right_major = (st.session_state.editing_problem_table == (which_table, right_key, 'major') if right_key else False)
        editing_right_minor = (st.session_state.editing_problem_table == (which_table, right_key, 'minor') if right_key else False)
        ...
        # Left side CR cell
        if left_key and editing_left_cr and is_editable:
            table_html += f'''
            <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #fee2e2; font-weight: 600;" class="problem-cell-editing">
                <input type="number" value="{problem_data[left_key]['cr']}" 
                onchange="handleProblemEdit('{which_table}', '{left_key}', 'cr', this.value)" 
                style="width: 60px; text-align: center; font-weight: bold; font-size: 14px; padding: 4px; border: 2px solid #667eea; border-radius: 4px; background: white;">
            </td>'''
        else:
            table_html += f'''
            <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #fee2e2; font-weight: 600;" 
                onclick="{f'editProblemCell(\"{which_table}\", \"{left_key}\", \"cr\", {problem_data[left_key]["cr"]})' if is_editable and left_key else ''}">
                {problem_data[left_key]['cr'] if left_key else ''}
            </td>'''
        
        # Left side MAJOR cell
        if left_key and editing_left_major and is_editable:
            table_html += f'''
            <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #ffedd5; font-weight: 600;" class="problem-cell-editing">
                <input type="number" value="{problem_data[left_key]['major']}" 
                onchange="handleProblemEdit('{which_table}', '{left_key}', 'major', this.value)" 
                style="width: 60px; text-align: center; font-weight: bold; font-size: 14px; padding: 4px; border: 2px solid #667eea; border-radius: 4px; background: white;">
            </td>'''
        else:
            table_html += f'''
            <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #ffedd5; font-weight: 600;" 
                onclick="{f'editProblemCell(\"{which_table}\", \"{left_key}\", \"major\", {problem_data[left_key]["major"]})' if is_editable and left_key else ''}">
                {problem_data[left_key]['major'] if left_key else ''}
            </td>'''
        
        # Left side MINOR cell
        if left_key and editing_left_minor and is_editable:
            table_html += f'''
            <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #dbeafe; font-weight: 600;" class="problem-cell-editing">
                <input type="number" value="{problem_data[left_key]['minor']}" 
                onchange="handleProblemEdit('{which_table}', '{left_key}', 'minor', this.value)" 
                style="width: 60px; text-align: center; font-weight: bold; font-size: 14px; padding: 4px; border: 2px solid #667eea; border-radius: 4px; background: white;">
            </td>'''
        else:
            table_html += f'''
            <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #dbeafe; font-weight: 600;" 
                onclick="{f'editProblemCell(\"{which_table}\", \"{left_key}\", \"minor\", {problem_data[left_key]["minor"]})' if is_editable and left_key else ''}">
                {problem_data[left_key]['minor'] if left_key else ''}
            </td>'''
        ...
        if right_key:
            ...
            # Right side CR cell
            if editing_right_cr and is_editable:
                table_html += f'''
                <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #fee2e2; font-weight: 600;" class="problem-cell-editing">
                    <input type="number" value="{problem_data[right_key]['cr']}" 
                    onchange="handleProblemEdit('{which_table}', '{right_key}', 'cr', this.value)" 
                    style="width: 60px; text-align: center; font-weight: bold; font-size: 14px; padding: 4px; border: 2px solid #667eea; border-radius: 4px; background: white;">
                </td>'''
            else:
                table_html += f'''
                <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #fee2e2; font-weight: 600;" 
                    onclick="{f'editProblemCell(\"{which_table}\", \"{right_key}\", \"cr\", {problem_data[right_key]["cr"]})' if is_editable else ''}">
                    {problem_data[right_key]['cr']}
                </td>'''
            
            # Right side MAJOR cell
            if editing_right_major and is_editable:
                table_html += f'''
                <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #ffedd5; font-weight: 600;" class="problem-cell-editing">
                    <input type="number" value="{problem_data[right_key]['major']}" 
                    onchange="handleProblemEdit('{which_table}', '{right_key}', 'major', this.value)" 
                    style="width: 60px; text-align: center; font-weight: bold; font-size: 14px; padding: 4px; border: 2px solid #667eea; border-radius: 4px; background: white;">
                </td>'''
            else:
                table_html += f'''
                <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #ffedd5; font-weight: 600;" 
                    onclick="{f'editProblemCell(\"{which_table}\", \"{right_key}\", \"major\", {problem_data[right_key]["major"]})' if is_editable else ''}">
                    {problem_data[right_key]['major']}
                </td>'''
            
            # Right side MINOR cell
            if editing_right_minor and is_editable:
                table_html += f'''
                <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #dbeafe; font-weight: 600;" class="problem-cell-editing">
                    <input type="number" value="{problem_data[right_key]['minor']}" 
                    onchange="handleProblemEdit('{which_table}', '{right_key}', 'minor', this.value)" 
                    style="width: 60px; text-align: center; font-weight: bold; font-size: 14px; padding: 4px; border: 2px solid #667eea; border-radius: 4px; background: white;">
                </td>'''
            else:
                table_html += f'''
                <td style="text-align: center; padding: 8px; border: 1px solid #e5e7eb; background-color: #dbeafe; font-weight: 600;" 
                    onclick="{f'editProblemCell(\"{which_table}\", \"{right_key}\", \"minor\", {problem_data[right_key]["minor"]})' if is_editable else ''}">
                    {problem_data[right_key]['minor']}
                </td>'''
    ...
    # Add JavaScript for editing
    table_html += '''
    <script>
    function editProblemCell(tableType, problemKey, severity, currentValue) {
        // Only allow editing for QC table
        if (tableType === 'qc') {
            ...
            input.type = 'hidden';
            input.name = 'edit_problem';
            input.value = tableType + '|' + problemKey + '|' + severity + '|' + currentValue;
    ...
    function handleProblemEdit(tableType, problemKey, severity, newValue) {
        // Only allow editing for QC table
        if (tableType === 'qc') {
            // Create a form to submit the edit
            const form = document.createElement('form');
            ...
            const input = document.createElement('input');
            input.type = 'hidden';
            input.name = 'update_problem';
            input.value = tableType + '|' + problemKey + '|' + severity + '|' + newValue;
            form.appendChild(input);
    ...
    if is_editable:
        ...
        if st.button(f"Update {problem_names[selected_problem]} - {t(selected_severity.upper())}", key=f"update_btn_{which_table}"):
