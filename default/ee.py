...
def create_photos_of_faults_table(language, chinese_font=None):
    """Create Photos of Faults table for PDF with QC Manager images"""
    # Get QC defect containers
    qc_containers = st.session_state.get('qc_defect_containers', [])
    
    # Group containers by severity
    critical_containers = [c for c in qc_containers if c.get('severity') == 'critical']
    major_containers = [c for c in qc_containers if c.get('severity') == 'major']
    minor_containers = [c for c in qc_containers if c.get('severity') == 'minor']
    ...
    if language == "Mandarin":
        ...
        # Add MINOR section if there are minor containers
        if minor_containers:
            data.append(['MINOR 次要问题', ''])
            for i, container in enumerate(minor_containers, 1):
                if i % 2 == 1:
                    # Left column
                    if i + 1 <= len(minor_containers):
                        data.append([f"{i}  {minor_containers[i-1]['name']}", 
                                   f"{i+1}  {minor_containers[i]['name']}"])
                    else:
                        data.append([f"{i}  {minor_containers[i-1]['name']}", ""])
                    # Add 3 rows for photo spaces
                    for _ in range(3):
                        data.append(['', ''])
        
        # Add MAJOR section if there are major containers
        if major_containers:
            data.append(['MAJOR 主要问题', ''])
            for i, container in enumerate(major_containers, 1):
                if i % 2 == 1:
                    # Left column
                    if i + 1 <= len(major_containers):
                        data.append([f"{i}  {major_containers[i-1]['name']}", 
                                   f"{i+1}  {major_containers[i]['name']}"])
                    else:
                        data.append([f"{i}  {major_containers[i-1]['name']}", ""])
                    # Add 3 rows for photo spaces
                    for _ in range(3):
        ...
        # Add CRITICAL section if there are critical containers
        if critical_containers:
            data.append(['CRITICAL 严重问题', ''])
            for i, container in enumerate(critical_containers, 1):
                if i % 2 == 1:
                    # Left column
                    if i + 1 <= len(critical_containers):
                        data.append([f"{i}  {critical_containers[i-1]['name']}", 
                                   f"{i+1}  {critical_containers[i]['name']}"])
                    else:
                        data.append([f"{i}  {critical_containers[i-1]['name']}", ""])
                    # Add 3 rows for photo spaces
                    for _ in range(3):
        ...
        # If no containers, add empty sections
    ...
    row_heights = [25]  # Title row
    
    for row in data[1:]:
        if 'MINOR' in row[0] or 'MAJOR' in row[0] or 'CRITICAL' in row[0]:
            row_heights.append(20)  # Section headers
        elif row[0] and row[1]:  # Both columns have defect names
            row_heights.append(20)
        elif not row[0] and not row[1]:  # Photo space rows
            row_heights.append(50)
        else:  # Single defect name
            row_heights.append(20)
    ...
    for i, row in enumerate(data[1:], 1):
        ...
        if 'MINOR' in row[0]:
            ...
            style.add('BACKGROUND', (0, row_idx), (1, row_idx), colors.Color(0.9, 0.9, 0.9))
            style.add('SPAN', (0, row_idx), (1, row_idx))
            style.add('FONTSIZE', (0, row_idx), (1, row_idx), 11)
            style.add('FONTNAME', (0, row_idx), (1, row_idx), chinese_font if language == "Mandarin" and chinese_font else 'Helvetica-Bold')
            style.add('ALIGN', (0, row_idx), (1, row_idx), 'LEFT')
            style.add('VALIGN', (0, row_idx), (1, row_idx), 'MIDDLE')
            sections_added += 1
        elif 'MAJOR' in row[0]:
            # MAJOR header - orange
            style.add('BACKGROUND', (0, row_idx), (1, row_idx), colors.Color(1, 0.7, 0.5))
            style.add('SPAN', (0, row_idx), (1, row_idx))
            style.add('FONTSIZE', (0, row_idx), (1, row_idx), 11)
            style.add('FONTNAME', (0, row_idx), (1, row_idx), chinese_font if language == "Mandarin" and chinese_font else 'Helvetica-Bold')
            style.add('ALIGN', (0, row_idx), (1, row_idx), 'LEFT')
            style.add('VALIGN', (0, row_idx), (1, row_idx), 'MIDDLE')
            sections_added += 1
        elif 'CRITICAL' in row[0]:
            # CRITICAL header - red
            style.add('BACKGROUND', (0, row_idx), (1, row_idx), colors.Color(0.9, 0.2, 0.2))
            style.add('SPAN', (0, row_idx), (1, row_idx))
            style.add('FONTSIZE', (0, row_idx), (1, row_idx), 11)
            style.add('FONTNAME', (0, row_idx), (1, row_idx), chinese_font if language == "Mandarin" and chinese_font else 'Helvetica-Bold')
            ...
            style.add('FONTSIZE', (0, row_idx), (1, row_idx), 10)
            style.add('FONTNAME', (0, row_idx), (1, row_idx), chinese_font if language == "Mandarin" and chinese_font else 'Helvetica')
            style.add('ALIGN', (0, row_idx), (1, row_idx), 'LEFT')
            style.add('VALIGN', (0, row_idx), (1, row_idx), 'TOP')
    
    # Add grid and other styles
    style.add('GRID', (0, 0), (1, len(data)-1), 1.5, colors.black)
    style.add('LINEWIDTH', (0, 0), (1, len(data)-1), 1.5)
    style.add('LINECOLOR', (0, 0), (1, len(data)-1), colors.black)
    style.add('LEFTPADDING', (0, 0), (1, len(data)-1), 5)
    style.add('RIGHTPADDING', (0, 0), (1, len(data)-1), 5)
    style.add('TOPPADDING', (0, 0), (1, len(data)-1), 3)
    style.add('BOTTOMPADDING', (0, 0), (1, len(data)-1), 3)
