...
def create_photos_of_faults_table(language, chinese_font=None):
    ...
    for severity in ['minor', 'major', 'critical']:
        ...
        for container in severity_containers:
            ...
            # Create defect name row - separate from image table
            defect_name = f"{container_counter}. {container['name']}"
            container_counter += 1
            
            # Add defect name as a separate paragraph
            defect_name_style = ParagraphStyle(
                'DefectName', 
            ...
            elements.append(Paragraph(defect_name, defect_name_style))
            
            # Split images into rows (2 images per row)
            for i in range(0, len(images), 2):
                image_pair = images[i:i+2]
                row_images = []
                
                for image_bytes in image_pair:
                    try:
                        # Create ReportLab Image from bytes
                        img_buffer = io.BytesIO(image_bytes)
                        reportlab_img = ReportLabImage(img_buffer)
                        
                        # Set image size - slightly smaller than cell to ensure it fits within borders
                        img_width = 2.2*cm  # About 0.86 inches
                        img_height = 2.2*cm  # About 0.86 inches
                        reportlab_img.drawWidth = img_width
                        reportlab_img.drawHeight = img_height
                        
                        row_images.append(reportlab_img)
                        
                    except Exception as e:
                        # If image fails, add placeholder text
                        st.warning(f"Error processing image: {str(e)}")
                        row_images.append(Paragraph('Image Error', ParagraphStyle(
                            'ErrorText',
                            fontName='Helvetica',
                            fontSize=8,
                            alignment=TA_CENTER,
                ...
                # Ensure we always have 2 items per row
                while len(row_images) < 2:
                    row_images.append('')  # Empty cell
                
                # Calculate column widths - 2 equal columns
                col_width = page_width * 0.5
                row_height = 2.5*cm  # Enough space for images
                
                # Create table for this row of images
                row_table = Table([row_images], colWidths=[col_width, col_width], rowHeights=[row_height])
                
                # Style the table to ensure images stay within borders
