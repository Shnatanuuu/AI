...
if 'component_value' in st.session_state and st.session_state.component_value:
    ...
    if data and data.get('type') == 'save_annotated_container':
        process_saved_annotation(data)
        st.session_state.component_value = None
        st.rerun()
# Simple annotation handler at TOP LEVEL (not inside any function)
if 'component_value' in st.session_state and st.session_state.component_value:
    annotation_data = st.session_state.component_value
    if annotation_data and annotation_data.get('type') == 'save_annotated_container':
        if process_saved_annotation(annotation_data):
            st.session_state.component_value = None
            st.rerun()
# Enhanced message handler for component communication - MUST BE AT TOP
# Enhanced message handler for component communication - MUST BE AT TOP
if 'component_handler_initialized' not in st.session_state:
    components.html("""
    <script>
    window.addEventListener('message', function(event) {
        console.log('Message received in handler:', event.data);
        
        // Handle annotation save messages
        if (event.data.type === 'save_annotated_container' || event.data.type === 'save_annotation') {
            console.log('Forwarding annotation data to Streamlit');
    ...
    console.log('Streamlit annotation message handler ready');
    </script>
    """, height=0)
    st.session_state.component_handler_initialized = True
load_dotenv()
...
def create_photos_of_faults_table(language, chinese_font=None):
    """Create Photos of Faults section for PDF with annotated images"""
    ...
    # Define page width for calculations
    ...
    for severity in ['critical', 'major', 'minor']:
        ...
        if severity != 'critical' or elements:
            elements.append(PageBreak())
        
        # Severity header
        header_style = ParagraphStyle(
            f'SeverityHeader{severity}', 
            fontName='Helvetica-Bold' if language == "English" else (chinese_font if chinese_font else 'Helvetica-Bold'),
            fontSize=14, 
        ...
        for container in containers_with_images:
            ...
            if images:
                try:
                    ...
                    # Set image size to fit on page
                    img_width = page_width * 0.7  # 70% of page width
...
def process_saved_annotation(annotation_data):
    """Simple annotation processor"""
    try:
        session_id = annotation_data.get('session_id')
        ...
        if session_key in st.session_state:
            ...
            container = {
                ...
                'images': [session_data['image_bytes']],
                'timestamp': datetime.now().isoformat()
            }
            ...
            # Show success
            st.success(f"✅ Container '{container['name']}' saved!")
...
def create_annotation_interface(image, image_bytes):
    """Create annotation interface with circle drawing - SIMPLE VERSION"""
    img_width, img_height = image.size
    
    # Create annotated image with a red circle
    annotated_image = image.copy()
    draw = ImageDraw.Draw(annotated_image)
    
    # Calculate circle position (center of image)
    center_x = img_width // 2
    center_y = img_height // 2
    radius = min(img_width, img_height) // 6  # 1/6th of smaller dimension
    ...
    st.image(annotated_image, caption="Defect marked with red circle", use_container_width=True)
    
    # Manual save option
    col1, col2 = st.columns([1, 1])
    
    with col1:
        if st.button("💾 **SAVE WITH ANNOTATION**", type="primary", use_container_width=True):
    ...
    with col2:
        if st.button("📷 **SAVE WITHOUT ANNOTATION**", use_container_width=True):
...
def cleanup_annotation_temp_data(session_key):
    """Clean up temporary annotation data but keep form state"""
    keys_to_delete = [
        session_key,
        'temp_uploaded_file',
        'temp_image_bytes',
