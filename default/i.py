...
def create_photos_of_faults_table(language, chinese_font=None):
    """Create Photos of Faults table for PDF with actual QC Manager images - WITH COLORED SECTIONS"""
    ...
    # Define background colors for each severity (light pastel colors)
    severity_colors = {
    ...
    # Define border colors for each severity (darker versions)
    ...
    for severity in ['minor', 'major', 'critical']:
        ...
        # Skip if no containers with images in this severity
        containers_with_images = [c for c in severity_containers if c.get('images')]
        if not containers_with_images:
            continue
        
        # Create a wrapper table for the entire severity section with border
        severity_wrapper_data = []
        
        # Row 1: Severity header with background color
        ...
        container_counter = 1  # Reset counter for each severity group
        ...
        for container in containers_with_images:
            ...
            # Create the numbered defect name (e.g., "1. Color variation")
            numbered_defect_name = f"{container_counter}. {defect_name}"
            container_counter += 1
            
            # Create a table for each defect (name + images)
            defect_table_data = []
            ...
            if images:
                image_cells = []
                img_cell_width = 3.0*cm  # Width for each image box
                img_cell_height = 3.0*cm  # Height for each image box
                ...
                for img_idx, img_bytes in enumerate(images):
                    try:
                        ...
                        # Set image size smaller than cell to show border
                        img_size = 2.5*cm  # Image is 2.5cm, cell is 3.0cm
                        reportlab_img.drawWidth = img_size
                        reportlab_img.drawHeight = img_size
                        
                        # Create a cell with the image centered
                        img_cell_table = Table([[reportlab_img]], 
                                              colWidths=[img_cell_width],
                                              rowHeights=[img_cell_height])
                        
                        # Style the image cell with border
                        img_cell_table.setStyle(TableStyle([
                            ('ALIGN', (0, 0), (0, 0), 'CENTER'),
                            ('VALIGN', (0, 0), (0, 0), 'MIDDLE'),
                            ('BOX', (0, 0), (0, 0), 0.5, colors.grey),  # Border around the image box
                        ...
                        image_cells.append(img_cell_table)
                        
                    except Exception as e:
                        # If image fails, add placeholder in a box
                        error_cell = Table([[Paragraph('X', ParagraphStyle(
                            'ErrorText',
                            fontName='Helvetica',
                            fontSize=8,
                            alignment=TA_CENTER,
                        ...
                        error_cell.setStyle(TableStyle([
                        ...
                        image_cells.append(error_cell)
                
                # Add images to the defect table
                if image_cells:
                    # Group images in rows of up to 3
                    max_images_per_row = min(3, len(image_cells))
                    
                    for i in range(0, len(image_cells), max_images_per_row):
                        row_images = image_cells[i:i + max_images_per_row]
                        # Create a row with images aligned to the left
                        images_row = Table([row_images], 
                                         colWidths=[img_cell_width] * len(row_images),
                                         hAlign='LEFT')
                        
                        images_row.setStyle(TableStyle([
                            ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                            ('LEFTPADDING', (0, 0), (0, 0), 0),  # No left padding on first image
                            ('RIGHTPADDING', (-1, 0), (-1, 0), 0),  # No right padding on last image
                        ]))
                        
                        defect_table_data.append([images_row])
            
            # Create the defect table
            defect_table = Table(defect_table_data, 
                               colWidths=[page_width],  # Full width
                               hAlign='LEFT')  # Left align the entire defect
            
            defect_table.setStyle(TableStyle([
                ('LEFTPADDING', (0, 0), (-1, -1), 10),  # Padding inside defect section
            ...
            # Add minimal spacing between defects within the same severity
            if container != containers_with_images[-1]:  # Not the last defect
                severity_wrapper_data.append([Spacer(1, 5)])
        
        # Create the severity wrapper table
        severity_wrapper = Table(severity_wrapper_data,
                               colWidths=[page_width])
        
        # Style the severity wrapper with background color and border
        severity_wrapper.setStyle(TableStyle([
            # Background color for the entire section
            ('BACKGROUND', (0, 0), (-1, -1), severity_colors[severity]),
            
            # Border around the entire severity section
            ('BOX', (0, 0), (-1, -1), 1.5, severity_border_colors[severity]),
            
            # Inner grid (light lines between rows)
            ('INNERGRID', (0, 0), (-1, -1), 0.5, severity_border_colors[severity]),
            
            # Header row styling
            ('BACKGROUND', (0, 0), (-1, 0), severity_border_colors[severity]),  # Darker background for header
...
def resize_image_for_pdf(image_bytes, max_width=2.2*cm, max_height=2.2*cm):
    """Resize image to fit within dimensions for PDF"""
    try:
        ...
        # Convert to inches for PIL (1 inch = 2.54 cm)
        max_width_inches = max_width / 2.54
        max_height_inches = max_height / 2.54
        
        # Calculate dimensions in pixels (assuming 150 DPI for PDF)
        dpi = 150
        max_width_pixels = int(max_width_inches * dpi)
        max_height_pixels = int(max_height_inches * dpi)
        
        # Resize image
        img_resized = img.resize((max_width_pixels, max_height_pixels), Image.Resampling.LANCZOS)
...
def render_photos_of_faults_ui():
    """Render Photos of Faults UI for QC Manager to add defect containers with images"""
    ...
    if st.session_state.qc_defect_containers:
        for idx, container in enumerate(st.session_state.qc_defect_containers):
            ...
            # Display images in this container
            st.markdown(f"**{t('defect_images')}:**")
            if container['images']:
                cols = st.columns(min(3, len(container['images'])))
