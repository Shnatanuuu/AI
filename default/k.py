...
def create_photos_of_faults_table(language, chinese_font=None):
    ...
    - Exactly 1 inch x 1 inch images
    - Proper numbering (restarts at 1 for each severity)
    - Yellow backgrounds with black borders
    - Severity-colored headers
    """
    from reportlab.platypus import Image as ReportLabImage, PageBreak, Spacer, Table, TableStyle, Paragraph
    ...
    # Create title table with yellow background
    title_table = Table(
    ...
    # ========== SEVERITY LABELS ==========
    severity_labels = {
        'critical': {"English": "CRITICAL DEFECTS", "Mandarin": "严重问题"},
        'major': {"English": "MAJOR DEFECTS", "Mandarin": "主要问题"},
        'minor': {"English": "MINOR DEFECTS", "Mandarin": "次要问题"}
    }
    
    # Severity colors for headers
    severity_colors = {
        'critical': colors.red,
        'major': colors.orange,
        'minor': colors.blue
    ...
    for severity in ['critical', 'major', 'minor']:
        ...
        # ========== SEVERITY HEADER ==========
        header_text = severity_labels[severity][language]
        ...
        # ========== PROCESS EACH CONTAINER (NUMBERING RESTARTS AT 1) ==========
        for container_num, container in enumerate(containers_with_images, start=1):
            ...
            # Create numbered defect name (numbering per severity type)
            numbered_defect_name = f"{container_num}. {defect_name}"
            
            # ========== DEFECT NAME TABLE ==========
            name_style = ParagraphStyle(
                f'DefectName_{severity}_{container_num}',
                fontName='Helvetica-Bold' if language == "English" else (chinese_font if chinese_font else 'Helvetica-Bold'),
                fontSize=12,
                alignment=TA_LEFT,
                textColor=colors.black,
            ...
            name_table = Table(
                [[Paragraph(numbered_defect_name, name_style)]],
                colWidths=[A4[0] - 4*cm],
                rowHeights=[25]
            )
            ...
            if images:
                try:
                    ...
                    # CRITICAL: Set to EXACTLY 1 inch x 1 inch
                    reportlab_img.drawWidth = 1 * inch
                    reportlab_img.drawHeight = 1 * inch
                    
                    # Create image container with yellow background
                    img_container = Table(
                        [[reportlab_img]],
                        colWidths=[1.2 * inch],
                        rowHeights=[1.2 * inch]
...
def render_photos_of_faults_ui():
    """Render Photos of Faults UI with proper save/add workflow - FIXED VERSION"""
    ...
    # Always show the form by default
    if 'show_add_container_form' not in st.session_state:
        st.session_state.show_add_container_form = True
    
    # Display existing containers FIRST
    if st.session_state.qc_defect_containers:
        st.markdown("#### 📦 Saved Defect Containers")
        ...
        for idx, container in enumerate(st.session_state.qc_defect_containers):
            ...
            color = severity_colors[container['severity']]
            ...
            with st.container():
                ...
                with col1:
                    ...
                    <div style="border-left: 6px solid {color}; 
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <h4 style="margin: 0; font-weight: bold;">{container['name']}</h4>
                            <span style="background: {color}; color: white; padding: 4px 12px; 
                                       border-radius: 12px; font-size: 0.8rem; font-weight: bold;">
                                {t(container['severity'].upper())}
                            </span>
                        </div>
                    </div>
                    ''', unsafe_allow_html=True)
                ...
                with col2:
                    # Edit button - FIXED: Use a unique key
    ...
    # "Create New Container" button (ALWAYS SHOW THIS)
    st.markdown("---")
    col1, col2 = st.columns([1, 1])
    
    with col1:
        if st.button("**➕ CREATE NEW CONTAINER**", type="primary", use_container_width=True, key="create_new_btn"):
            st.session_state.show_add_container_form = True
    ...
    with col2:
        if st.button("📋 View All Containers", use_container_width=True, key="view_all_btn"):
            if st.session_state.qc_defect_containers:
                st.write(f"### Total containers: {len(st.session_state.qc_defect_containers)}")
                for i, c in enumerate(st.session_state.qc_defect_containers, 1):
                    st.write(f"{i}. **{c['name']}** ({c['severity']}) - {len(c.get('images', []))} image(s)")
            else:
                st.info("No containers saved yet")
    
    # Show add/edit container form
    if st.session_state.show_add_container_form:
        # SAFELY get the edit index
        ...
        if has_name and has_image:
            ...
            with col2:
                # Manual save button as backup - FIXED: Use proper edit index
                if st.button("💾 **MANUAL SAVE CONTAINER**", type="primary", use_container_width=True, key="manual_save_btn"):
                    # Create container
                    container = {
                    ...
                    # Check if editing existing container - FIXED with validation
                    if is_editing and isinstance(edit_idx, int) and 0 <= edit_idx < len(st.session_state.qc_defect_containers):
                        # Update existing container
                        st.session_state.qc_defect_containers[edit_idx] = container
                        st.success(f"✅ Updated container: {container['name']}")
                    else:
                        # Add new container
                        st.session_state.qc_defect_containers.append(container)
                        st.success(f"✅ Saved new container: {container['name']}")
                    
                    # Clear form and show success
                    st.session_state.show_add_container_form = True  # Keep form open
                    for key in ['temp_container_name', 'temp_severity',
                               'temp_image_bytes', 'temp_image', 'editing_existing_container_idx']:
                        if key in st.session_state:
                            del st.session_state[key]
            ...
            # Show instructions
            st.info("👆 Fill in the container name, select severity, and upload an image to get started")

def create_annotation_interface(image, image_bytes):
    """Create compact annotation interface with sliders on right, image on left"""
    img_width, img_height = image.size
    
    # Initialize circle position in session state if not exists
    if 'circle_x_percent' not in st.session_state:
        st.session_state.circle_x_percent = 50  # Start at center
    if 'circle_y_percent' not in st.session_state:
        st.session_state.circle_y_percent = 50  # Start at center
    if 'circle_radius_percent' not in st.session_state:
        st.session_state.circle_radius_percent = 10  # Default radius
    
    # Calculate actual pixel positions
    x_position = st.session_state.circle_x_percent
    y_position = st.session_state.circle_y_percent
    radius = st.session_state.circle_radius_percent
    
    center_x = int(img_width * x_position / 100)
    center_y = int(img_height * y_position / 100)
    actual_radius = int(min(img_width, img_height) * radius / 100)
    
    # Create annotated image with circle at specified position
    ...
    for width in range(8, 0, -2):
        draw.ellipse(
            [center_x - actual_radius - width, center_y - actual_radius - width, 
             center_x + actual_radius + width, center_y + actual_radius + width],
            outline='red',
            width=3
        )
    ...
    # Create two columns: Image on left, Controls on right
    ...
    with col_controls:
        ...
        st.info("🎯 Adjust sliders to position circle")
        ...
        x_position = st.slider(
            ...
            min_value=0,
            max_value=100,
            value=st.session_state.circle_x_percent,
            step=1,
            key="circle_x_slider",
            help="Move circle horizontally"
        )
        st.session_state.circle_x_percent = x_position
        
        y_position = st.slider(
            "↑ Up / Down ↓",
            min_value=0,
            max_value=100,
            value=st.session_state.circle_y_percent,
            step=1,
            key="circle_y_slider",
            help="Move circle vertically"
        )
        st.session_state.circle_y_percent = y_position
        
        radius = st.slider(
            "⭕ Circle Size",
            min_value=5,
            max_value=30,
            value=st.session_state.circle_radius_percent,
            step=1,
            key="circle_radius_slider",
            help="Adjust circle size"
        )
        st.session_state.circle_radius_percent = radius
        
        st.markdown("---")
        
        # Single SAVE CONTAINER button
        if st.button("💾 **SAVE CONTAINER**", type="primary", use_container_width=True, key="save_annotated_container"):
            ...
            # Create container
            container = {
                'name': st.session_state.get('temp_container_name', 'Unnamed'),
                'severity': st.session_state.get('temp_severity', 'minor'),
            ...
            if 'editing_existing_container_idx' in st.session_state:
                ...
                if isinstance(idx, int) and 0 <= idx < len(st.session_state.qc_defect_containers):
                    ...
                    st.success(f"✅ Updated: {container['name']}")
                else:
                    st.session_state.qc_defect_containers.append(container)
                    st.success(f"✅ Saved: {container['name']}")
            else:
                st.session_state.qc_defect_containers.append(container)
                st.success(f"✅ Saved: {container['name']}")
            
            # Clear temp data and reset circle position
            for key in ['temp_container_name', 'temp_severity',
                       'temp_image_bytes', 'temp_image', 'editing_existing_container_idx',
                       'circle_x_percent', 'circle_y_percent', 'circle_radius_percent']:
                if key in st.session_state:
                    del st.session_state[key]
...
def generate_multilingual_pdf(order_info, language):
    ...
    try:
        ...
        # 🔴🔴🔴 ENHANCED PHOTOS OF FAULTS SECTION 🔴🔴🔴
        # Check if we have any defect containers
        qc_containers = st.session_state.get('qc_defect_containers', [])
        
        if qc_containers:
            # Start new page for Photos of Faults
            elements.append(PageBreak())
            
            # Photos of Faults Title with styling
            if language == "Mandarin":
                photos_title = "缺陷照片"
            else:
                photos_title = "PHOTOS OF FAULTS"
            ...
            title_table_data = [[Paragraph(photos_title, photos_title_style)]]
            title_table = Table(title_table_data, colWidths=[A4[0] - 4*cm])
            title_table.setStyle(TableStyle([
            ...
            # Add each container with beautiful styling
            for idx, container in enumerate(qc_containers, 1):
                container_name = container['name']
                ...
                severity_colors = {
                    ...
                    'major': colors.Color(0.95, 0.6, 0.2),    # Orange
                    'minor': colors.Color(0.2, 0.5, 0.9)      # Blue
                }
                
                severity_backgrounds = {
                    'critical': colors.Color(1.0, 0.9, 0.9),  # Light red
                    'major': colors.Color(1.0, 0.95, 0.9),    # Light orange
                    'minor': colors.Color(0.9, 0.95, 1.0)     # Light blue
                }
                
                # Translate name if needed
                if language == "Mandarin":
                    container_name = translate_text_with_openai(container_name, "Mandarin")
                
                # Container header with severity - Create a nice box
                severity_text = t(severity.upper())
                header_text = f"<b>{idx}. {container_name}</b>"
                ...
                header_table = Table(header_table_data, colWidths=[10*cm, 3*cm])
                header_table.setStyle(TableStyle([
                    ('BACKGROUND', (0, 0), (0, 0), severity_backgrounds.get(severity, colors.white)),
                    ('BACKGROUND', (1, 0), (1, 0), severity_colors.get(severity, brand_blue)),
                    ('TEXTCOLOR', (0, 0), (0, 0), severity_colors.get(severity, colors.black)),
                    ('TEXTCOLOR', (1, 0), (1, 0), colors.white),
                    ('ALIGN', (0, 0), (0, 0), 'LEFT'),
                    ('ALIGN', (1, 0), (1, 0), 'CENTER'),
                    ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
                    ('BOX', (0, 0), (-1, -1), 1, severity_colors.get(severity, colors.grey)),
                ...
                # Add image if available with border and background
                if images:
                    try:
                        # Create a container box for the image
                        img_buffer = io.BytesIO(images[0])
                        img = ReportLabImage(img_buffer)
                        
                        # Set smaller image size
                        img.drawWidth = 300  # Smaller width
                        img.drawHeight = 225  # Smaller height (3:4 ratio)
                        
                        # Create a table to hold the image with border
                        img_table_data = [[img]]
                        img_table = Table(img_table_data, colWidths=[320], rowHeights=[235])
                        img_table.setStyle(TableStyle([
                            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
                            ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
                            ('BACKGROUND', (0, 0), (-1, -1), colors.Color(0.98, 0.98, 0.98)),
                            ('BOX', (0, 0), (-1, -1), 2, severity_colors.get(severity, colors.grey)),
                        ...
                        # If image fails, add error message in a styled box
                        error_msg = f"⚠️ Image display error"
                        error_style = ParagraphStyle(
                            'ImageError',
                        ...
                        error_table_data = [[Paragraph(error_msg, error_style)]]
                        error_table = Table(error_table_data, colWidths=[320])
                ...
                # Add decorative separator between containers (except after last one)
                if idx < len(qc_containers):
                    # Add a decorative line
                    separator_table_data = [[""]]
                    separator_table = Table(separator_table_data, colWidths=[A4[0] - 4*cm], rowHeights=[2])
                    ...
                    # Add page break only if this isn't the last container
                    elements.append(PageBreak())
