...
def create_sampling_table(order_qty, language, chinese_font=None):
    """Create sampling inspection table for PDF - UPDATED FOR MANDARIN"""
    try:
        # Get sampling limits based on order quantity
...
def generate_multilingual_pdf(order_info, language):
    """Generate PDF with proper Mandarin font handling"""
    try:
        buffer = io.BytesIO()
        ...
        try:
            ...
            if language == "Mandarin":
                try:
                    # Try SimSun font (common Chinese font on Windows)
                    ...
                    try:
                        ...
                        # Try STSong (ReportLab built-in Chinese font)
        ...
        # PHOTOS OF FAULTS - QC MANAGER ONLY
        elements.append(PageBreak())
...
def render_defect_container_ui(container_type):
    """Render UI for defect containers (Photos of Faults) - ONLY QC MANAGER"""
    if container_type != "qc":
        return  # Only render QC containers
        ...
        for container_id, container in containers.items():
            ...
            if uploaded_files:
                for uploaded_file in uploaded_files:
                    ...
                    # Check if this image is already in the container
                    image_already_exists = False
                    for img_data in container['images']:
                        if isinstance(img_data, dict):
                            existing_bytes = img_data.get('bytes')
                        else:
                            existing_bytes = img_data
                        
                        # Simple check - compare first few bytes
                        if existing_bytes and existing_bytes[:100] == image_bytes[:100]:
                            image_already_exists = True
                            break
                    
                    if not image_already_exists:
                        # Store the bytes directly
                        container['images'].append(image_bytes)
                        st.rerun()
            ...
            if container['images']:
                cols = st.columns(3)
                for idx, img_bytes in enumerate(container['images']):
                    with cols[idx % 3]:
                        # Display the image
